...
BUILD_DIR = os.path.join(BUILD_DIR, "efficacy__analysis")
...
def run(
    ...
    if run_id == "shie":
        positions = [
            (u, v.split("__")[0] + f",{v.split('__')[1][:2]}")
            for u, v in positions
        ]

    if skip_figure: return a_mean, positions
    ...
    (
        ...
        *_
        ...
        a_mean, positions, negate=True, correction=correction, fig=fig,
    )
    fig, axes = fig
    ...
    if "big" in run_id:
        ...
        u, v = 2, 1
        t = stats.ttest_ind(
            a=a_mean[:, u], b=a_mean[:, v], nan_policy="omit"
        ).pvalue
...
def arrange(run_id, model_dir):
    ...
    if run_id in {"diam", "radii", "vertices"}:
        ...
        if "ground" in run_id:
            experiment = [(u, v) for u, v in experiment if "" in v.split("-")]
            experiment = [(u, v) for u, v in experiment if v[-1] == "L"]    # keep only lateral side
        elif "ML" in run_id:
            experiment = [
                (u, v) for u, v in experiment
            ...
            experiment = [
                ...
                and v.split("-")[0][-1] == "L"                      # keep only lateral side
            ]
        elif "RA" in run_id:
            experiment = [
                (u, v) for u, v in experiment
