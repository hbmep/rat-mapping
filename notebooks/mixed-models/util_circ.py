...
def plot_model(
    ...
    **kws,
    ...
    body_print_ci("# circ position", temp_labels, temp_beta, temp_se, decimals=1)
    ...
    body_print_ci("# circ distance", distance, mu_beta, se_beta, decimals=1)
    ...
    if orientation_style == "bars":
        ...
        mu = A @ fe.values
        se_or = np.sqrt(np.maximum(np.einsum("ij,jk,ik->i", A, cov, A), 0.0))
        lo = mu - SEM_TIMES * se_or
        hi = mu + SEM_TIMES * se_or
        ...
        body_print_ci("# circ orientation", np.array([0, 45, 90, 135]), mu, se_or, decimals=1)
        ...
        if "both" in orientation_style:
            ...
            body_print_ci("# circ orientation", temp3, mu, se_or, decimals=1)
            ...
            temp3 = np.array([np.rad2deg(t1) - 180])
            body_print_ci("# circ orientation", temp3, mu[1:], se_or[1:], decimals=1)
